...
export class GoogleSearchRunnable extends Runnable<
  Company,
  CompanyInfo,
  RunnableConfig
> {
  async invoke(input: Company): Promise<CompanyInfo> {
    try {
      const searchResults = await googleSearch.invoke(
        `${input.name} site:linkedin.com`
      );
      const parsedSearchResults: Array<{ link: string }> =
